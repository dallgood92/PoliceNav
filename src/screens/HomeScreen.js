...
export default function HomeScreen({ live, partners, department, squads, officer, duty, onDutyChange, onManageDepartment, onSelectPartner }) {
  return (
    <ScrollView contentContainerStyle={styles.content}>
      ...
      <DutyAssignmentCard officer={officer} partners={partners} assignment={duty} onChange={onDutyChange} />
      {live.error ? (
      ...
      <View style={styles.partnerList}>
        ...
        {!partners.length ? <Text style={styles.emptyPartners}>No squad partners are sharing a location yet.</Text> : null}
      </View>
...
const styles = StyleSheet.create({
  ...
  emptyPartners: { color: colors.muted, backgroundColor: colors.panel, borderColor: colors.border, borderWidth: 1, borderRadius: 10, paddingHorizontal: 16, paddingVertical: 20, textAlign: 'center', fontSize: 13, lineHeight: 18 },
