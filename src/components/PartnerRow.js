...
export default function PartnerRow({ partner, userLocation, onPress, compact = false, dense = false }) {
  ...
  const row = (
    <Pressable
      ...
      style={({ pressed }) => [styles.row, compact && styles.compactRow, dense && styles.denseRow, alertStyle, isPursuit && styles.pursuitRow, pressed && styles.pressed]}
    >
      <View style={[styles.badge, dense && styles.denseBadge, { backgroundColor: partner.avatarColor || '#059669' }, highlighted && styles.alertBadge]}>
        {crew.map((member, index) => (
          <View key={`badge-${member.callSign}`} style={styles.badgeLine}>
            {index ? <View style={styles.badgeDivider} /> : null}
            <Text style={[styles.badgeText, dense && styles.denseBadgeText]}>{member.callSign}</Text>
          </View>
        ))}
      </View>
      <View style={styles.identity}>
          ...
          <View style={styles.crewRow}>
            {crew.map((member) => (
              <View key={`${member.name}-${member.callSign}`} style={styles.crewMember}>
                <Text numberOfLines={1} style={[styles.name, dense && styles.denseName, highlighted && styles.alertInk]}>{member.name}</Text>
              </View>
            ))}
          </View>
        </View>
        ...
        <View style={[styles.presenceRow, dense && styles.densePresence]}>
      ...
      <Text style={[styles.distance, dense && styles.denseDistance, highlighted && styles.alertInk]}>{formatDistance(distance)}</Text>
      <Text style={[styles.chevron, highlighted && styles.alertInk]}>›</Text>
    </Pressable>
  );
  return isPursuit ? (
    <Animated.View style={[styles.pursuitWrapper, compact && styles.compactPursuitWrapper, dense && styles.densePursuitWrapper, { backgroundColor: pursuitPulse.interpolate({ inputRange: [0, 1], outputRange: ['#B91C2C', '#174EA6'] }) }]}>
      {row}
    </Animated.View>
  ) : row;
}
...
const styles = StyleSheet.create({
  ...
  denseRow: { minHeight: 55, paddingHorizontal: 10, paddingVertical: 4, marginBottom: 4 },
  ...
  densePursuitWrapper: { marginBottom: 4 },
  ...
  denseBadge: { width: 42, minHeight: 42, borderRadius: 7, paddingHorizontal: 5, paddingVertical: 3 },
