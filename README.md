...
Each officer can edit their current unit number, call sign, and optional second rider from the dashboard. Units support one or two displayed occupants. Status can be set to clear, traffic stop, or cover requested. Traffic stops use a yellow highlighted border; cover requests use red. A transition into cover-requested status sends a high-priority audible push alert to registered devices belonging to the requesting officer's squad partners, excluding the requesting device. Tapping the alert opens directions to the requesting officer's latest server location. iOS does not permit an app to launch Maps without user interaction while another app is active or the phone is locked.
